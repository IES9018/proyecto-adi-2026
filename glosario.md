...
- **E2E** (End-to-End) = Prueba que simula el recorrido completo de un usuario, desde abrir la app hasta que la operación se confirma.
- **Test case** = Caso de prueba: descripción concreta de una situación que se va a verificar.
- **Test suite** = Conjunto organizado de casos de prueba que se ejecutan juntos.
- **Assertion** (aserción) = Afirmación que verificá: "esto tiene que ser verdad". Si no lo es, el test falla.
- **Test oracle** (oráculo de prueba) = La fuente de verdad que define si el resultado es correcto o no.
- **Fixture** = Conjunto de datos y configuración preparada antes de ejecutar un test.
- **Seed** = Estado inicial conocido que se carga en el sistema antes de los tests.
- **Reset** = Acción de devolver el sistema a un estado limpio después de cada test.
- **Teardown** = Limpieza final después de que todos los tests terminaron.
- **Selector** = Identificador que Playwright usa para encontrar un elemento en la pantalla.
- **Trace** (traza) = Registro detallado de todo lo que pasó durante la ejecución de un test.
- **Flaky test** (test inestable) = Test que a veces pasa y a veces falla sin que el código haya cambiado.
- **Smoke test** (test de humo) = Prueba rápida que verifica que lo más básico funcione.
- **Happy path** (camino feliz) = El escenario donde todo sale bien, sin errores ni excepciones.
- **Secret** (secreto) = Credencial sensible que no debe exponerse: API key, contraseña, token JWT.
- **Secret scanning** = Herramienta que busca credenciales hardcodeadas en código y documentación.
- **Credential rotation** = Cambiar una contraseña o token que pudo haberse expuesto.
- **Redaction** (redacción) = Ocultar información sensible en reportes y logs.
